...
# (0lines,1lines,2lines,3lines,4lines,T Spin Double,T Spin Triple,T Spin Single,T Spin Mini, Perfect Clear, Back 2 Back)
...
BGCOLOR = BLACK
TEXTCOLOR = WHITE
COLORS = (GREEN, RED, BLUE, ORANGE, LIGHTBLUE, YELLOW, PINK, S_GREEN,
          S_RED, S_BLUE, S_ORANGE, S_LIGHTBLUE, S_YELLOW, S_PINK, GRAY3, GRAY4)
GAMEOVERCOLOR = GRAY3
...
def main():
    ...
    while True:  # game loop
        runGame()
        client.sendall(bytes("endgame", 'UTF-8'))
        showTextScreen('Game Over')


def runGame():
    # setup variables for the start of the game
    global isRunning, loseP1, loseP2, DISPLAYSURF, prevBoard, boardP1, boardP2, fallingPieceP1, fallingPieceP2, holdPieceP1, holdPieceP2, NEXT_PIECESP1, NEXT_PIECESP2, garbageQueueForP1, garbageQueueForP2, data
    data = ""
    isRunning = True
    boardP1 = getBlankBoard()
    prevBoard = getBlankBoard()
    ...
    NEXT_PIECESP1 = [nextPiece1P1, nextPiece2P1,
                     nextPiece3P1, nextPiece4P1, nextPiece5P1]
    holdPieceP1 = None

    NEXT_PIECESP2 = [nextPiece1P1, nextPiece2P1,
                     nextPiece3P1, nextPiece4P1, nextPiece5P1]

    holdPieceP2 = None
    client.sendall(bytes("ok", 'UTF-8'))
    while True:
        readyClient = select.select([client], [], [], 0.05)
        if readyClient[0]:
            start = client.recv(10000).decode()
            if start == "start":
                break
    timestart = time.time()
    framecount = 0
    while isRunning:  # game loop
        ...
        print(framecount/(time.time()-timestart))
        getOnlineData()
        ...
        for event in pygame.event.get():  # event handling loop
            if event.type == KEYUP:
                if (event.key == K_TAB):
                    ...
                    titleSurf, titleRect = makeTextObjs(
                        'Instructions', BIGFONT, TEXTCOLOR)
                    titleRect.center = (
                        int(WINDOWWIDTH / 2), int(WINDOWHEIGHT / 2) - 200)
            ...
            if event.type == KEYDOWN:
                ...
                # hold a peice
                    ...
                    holdPieceP1['x'] = int(
                        BOARDWIDTH / 2) - int(TEMPLATEWIDTH / 2)
                ...
                # rotating the piece (if there is room to rotate)
                    ...
                    if not isValidPosition(boardP1, fallingPieceP1, adjY=1):
                        lastFallTimeP1 = time.time() + DELAYLOCKIN - fallFreq
                    spinDirectionP1 = 'CW'
                    fallingPieceP1['rotation'] = (
                        fallingPieceP1['rotation'] + 1) % len(PIECES[fallingPieceP1['shape']])
                    if fallingPieceP1['shape'] == 'T':
                        if isValidPosition(boardP1, fallingPieceP1) and not isValidPosition(boardP1, fallingPieceP1, adjY=-1) and is3ConnerRule(boardP1, fallingPieceP1):
                            ...
                            superRotationSystem(
                                boardP1, fallingPieceP1, spinDirectionP1)
                    ...
                    if not isValidPosition(boardP1, fallingPieceP1, adjY=1):
                        lastFallTimeP1 = time.time() + DELAYLOCKIN - fallFreq
                    fallingPieceP1['rotation'] = (
                        fallingPieceP1['rotation'] + 2) % len(PIECES[fallingPieceP1['shape']])
                    if not isValidPosition(boardP1, fallingPieceP1):
                        fallingPieceP1['rotation'] = (
                            fallingPieceP1['rotation'] - 2) % len(PIECES[fallingPieceP1['shape']])
                ...
                # move the current piece all the way down
                    ...
                    lineSentToP2, P1backToBack, P1ComboCounter, P1LineRemoved, P1PerfectClear, P1backToBackPrint, P1Tetris, P1Combo, P1TMini, P1TSingle, P1TDouble, P1TTriple = sendLineFromP1(
                        boardP1, removeCompleteLines(boardP1), P1Tspin, P1backToBack, P1ComboCounter, P1PerfectClear, P1backToBackPrint, P1Tetris, P1Combo, P1TMini, P1TSingle, P1TDouble, P1TTriple)
                    if P1LineRemoved == 0:
        ...
        drawSpecialMove(60, 300, P1ComboCounter, P1Tspin, P1backToBackPrint, boardP1,
                        P1Tetris, P1PerfectClear, P1Combo, P1TMini, P1TSingle, P1TDouble, P1TTriple)
        drawSpecialMove(660, 300, P2ComboCounter, P2Tspin, P2backToBackPrint, boardP2,
                        P2Tetris, P2PerfectClear, P2Combo, P2TMini, P2TSingle, P2TDouble, P2TTriple)
...
def getOnlineData():
    ...
    if ready[0]:
        in_data = client.recv(10240)
        data += in_data.decode()
        if "endgame" in data:
            isRunning = False
        else:
            if data.rfind('[{"board":') != -1 and data.rfind('}]') != -1 and data.rfind('[{"board":') < data.rfind('"end": ""}]'):
                a = json.loads(data[data.rfind('[{"board":'):data.rfind('"end": ""}]')+11])[0]
                data = data[data.rfind('"end": ""}]')+11:]
                boardP2 = a["board"]
                fallingPieceP2 = a["piece"]
                holdPieceP2 = a["hold"]
                NEXT_PIECESP2 = a["next"]
                opQueue1 = a["queue1"]
                opQueue2 = a["queue2"]
                garbageQueueForP1 = opQueue2
                if opQueue1 == [] and garbageQueueForP2 != []:
                    garbageQueueForP2 = opQueue1


def pushOnlineData():
    global loseP1, loseP2, DISPLAYSURF, prevBoard, boardP1, boardP2, fallingPieceP1, fallingPieceP2, holdPieceP1, holdPieceP2, NEXT_PIECESP1, NEXT_PIECESP2, garbageQueueForP1, garbageQueueForP2
    board1 = [x[:] for x in boardP1]
    addToBoard(board1, fallingPieceP1)
    # addToBoard(board1, fallingPieceP1)
    if prevBoard != board1:
        dict1 = {"board": boardP1, "piece": fallingPieceP1, "hold": holdPieceP1,
                 "next": NEXT_PIECESP1, "queue1": garbageQueueForP1, "queue2": garbageQueueForP2, "end":""}
        client.sendall(bytes(json.dumps([dict1]), 'UTF-8'))
        prevBoard = board1
...
def superRotationSystem(board, fallingPiece, spinDirection):
    ...
    rotationAndDirection = [(fallingPiece['rotation'] == 1 and spinDirection == 'CW'),
                            (fallingPiece['rotation'] ==
                             0 and spinDirection == 'CCW'),
                            (fallingPiece['rotation'] ==
                             2 and spinDirection == 'CW'),
                            (fallingPiece['rotation'] ==
                             1 and spinDirection == 'CCW'),
                            (fallingPiece['rotation'] ==
                             3 and spinDirection == 'CW'),
                            (fallingPiece['rotation'] ==
                             2 and spinDirection == 'CCW'),
                            (fallingPiece['rotation'] ==
                             0 and spinDirection == 'CW'),
...
def drawBoxP1(boxx, boxy, color, pixelx=None, pixely=None):
    ...
    pygame.draw.rect(
        DISPLAYSURF, COLORS[color], (pixelx, pixely, BOXSIZE, BOXSIZE))
...
def drawBoardP1(board):
    ...
    for y in range(BOARDHEIGHT - 20):
        for x in range(BOARDWIDTH):
            pygame.draw.rect(DISPLAYSURF, GRAY1, (x*BOXSIZE + XMARGIN - 300, y *
                                                  BOXSIZE + TOPMARGIN + PLAYBOARD * BOXSIZE + 2, BOXSIZE, BOXSIZE), 1)
    # draw the border around the board
    pygame.draw.rect(DISPLAYSURF, BORDERCOLOR, (XMARGIN-302, TOPMARGIN + PLAYBOARD *
                                                BOXSIZE + 2, (BOARDWIDTH * BOXSIZE) + 4, (PLAYBOARD * BOXSIZE)), 3)
    ...
    for y in range(BOARDHEIGHT - 20):
        for x in range(BOARDWIDTH):
            pygame.draw.rect(DISPLAYSURF, GRAY1, (x*BOXSIZE + XMARGIN + 300, y *
                                                  BOXSIZE + TOPMARGIN + PLAYBOARD * BOXSIZE + 2, BOXSIZE, BOXSIZE), 1)
    # draw the border around the board
    pygame.draw.rect(DISPLAYSURF, BORDERCOLOR, (XMARGIN+298, TOPMARGIN + PLAYBOARD *
                                                BOXSIZE + 2, (BOARDWIDTH * BOXSIZE) + 4, (PLAYBOARD * BOXSIZE)), 3)
    # draw the individual boxes on the board
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT):
...
def drawHiddenBoardP1():
    pygame.draw.rect(DISPLAYSURF, BLACK, (XMARGIN-302, TOPMARGIN + 2,
                                          (BOARDWIDTH * BOXSIZE) + 10, (PLAYBOARD * BOXSIZE)))


def drawHiddenBoardP2():
    pygame.draw.rect(DISPLAYSURF, BLACK, (XMARGIN+298, TOPMARGIN + 2,
                                          (BOARDWIDTH * BOXSIZE) + 10, (PLAYBOARD * BOXSIZE)))
...
def drawAleartBarP1(receivedLine):
    ...
    pygame.draw.rect(DISPLAYSURF, BORDERCOLOR, (XMARGIN-99, TOPMARGIN +
                                                PLAYBOARD * BOXSIZE + 3, 8, (PLAYBOARD * BOXSIZE) - 1), 3)

    pygame.draw.rect(DISPLAYSURF, color, (XMARGIN-97, TOPMARGIN + PLAYBOARD *
                                          BOXSIZE + 3 + (BOXSIZE * (20 - receivedLine)), 4, BOXSIZE * receivedLine))
...
def drawAleartBarP2(receivedLine):
    ...
    pygame.draw.rect(DISPLAYSURF, BORDERCOLOR, (XMARGIN+501, TOPMARGIN +
                                                PLAYBOARD * BOXSIZE + 3, 8, (PLAYBOARD * BOXSIZE) - 1), 3)

    pygame.draw.rect(DISPLAYSURF, color, (XMARGIN+503, TOPMARGIN + PLAYBOARD *
                                          BOXSIZE + 3 + (BOXSIZE * (20 - receivedLine)), 4, BOXSIZE * receivedLine))
